...
export default function Login({ onLogin }) {
  ...
  useEffect(() => {
    let intervalo;
    if (modoLogin === 'qrcode') {
      fetch('http://72.60.3.89:8006/api/device/code')
        .then(res => res.json())
        .then(data => {
          setQrCode(data.code);
          intervalo = setInterval(() => {
            fetch(`http://72.60.3.89:8006/api/device/poll/${data.code}`)
              .then(res => res.json())
              .then(pollData => {
  ...
  const efetuarLoginQr = async (user, pass) => {
    setCarregando(true);
    try {
      const response = await fetch('http://72.60.3.89:8006/api/login', {
        method: 'POST',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = modoLogin === 'login' ? 'http://72.60.3.89:8006/api/login' : 'http://72.60.3.89:8006/api/register';
